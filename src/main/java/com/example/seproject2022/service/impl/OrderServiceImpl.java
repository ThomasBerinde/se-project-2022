...
public class OrderServiceImpl implements OrderService {
        ...
        public CreateOrderResponseDto createOrder(CreateOrderRequestDto createOrderRequestDto,
                ...
                if (createOrderRequestDto.getProductIds() == null || createOrderRequestDto.getProductIds()
                                                                                          .isEmpty()) {
                        throw new CustomException("You must have at least one product in your order",
                ...
                for (Long productId : createOrderRequestDto.getProductIds()) {
                        Product product = productRepository.findProductById(productId)
                                                           .orElse(null);
                        if (product == null) {
                                throw new CustomException(String.format("Product with id=%d doesn't exists", productId),
                                                          HttpStatus.NOT_FOUND, uri);
                        }
                        products.add(product);
                }
                createPdf(products);
                return new CreateOrderResponseDto("Successfully created order");
        }
        ...
        private void createPdf(List<Product> products) {
                try {
                        ...
                        PdfWriter.getInstance(document,
                                              new FileOutputStream("src/main/resources/orders/order-" + hash + ".pdf"));
                        document.open();
